...
@app.before_request
def chart_before():
    agent_email, agent_user = request.headers.get('X-Forwarded-Email', ''), request.headers.get('X-Forwarded-User', '')
    if not agent_email or not agent_user:
        return redirect('http://falcon-dashboard.s.upyun.com/', code=301)
